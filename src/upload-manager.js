...
class UploadManager {
  ...
  async initialize(input) {
    ...
    const mimeType = String(input.mimeType || 'application/octet-stream').slice(0, 150);
    const clientFingerprint = String(input.fingerprint || '').toLowerCase();
    const extension = path.extname(name).toLowerCase();

    if (!name || name.length > 255) throw new UploadError('Nome de arquivo inválido.');
    if (!this.allowedExtensions.has(extension)) {
      throw new UploadError('Formato não permitido. Envie arquivos ZIP, RAR, 7Z ou ISO.');
    }
    if (!Number.isSafeInteger(size) || size <= 0) throw new UploadError('Tamanho de arquivo inválido.');
    if (!/^[a-f0-9]{64}$/.test(clientFingerprint)) throw new UploadError('Identificação do arquivo inválida.');
    if (size > this.maxFileSize) {
      throw new UploadError(`O arquivo excede o limite de ${formatGigabytes(this.maxFileSize)}.`, 413);
    }

    const fingerprint = crypto.createHash('sha256')
      .update(`${name}\0${size}\0${lastModified}\0${clientFingerprint}`)
      .digest('hex');
    ...
    return { ...this.publicSession(session), uploadedChunks: [], resumed: false };
  }

  async status(id) {
    const session = await this.readSession(id);
    const uploadedChunks = session.status === 'active'
  ...
  async receiveChunk(id, indexValue, request) {
    const session = await this.readSession(id);
    if (session.status !== 'active') throw new UploadError('Este upload já foi finalizado.', 409);
    ...
    if (!Number.isSafeInteger(declaredSize) || declaredSize !== expectedSize) {
      throw new UploadError(`O bloco ${index} deveria ter ${expectedSize} bytes.`);
    }
    ...
    try {
      ...
      if (error.code === 'EEXIST') throw new UploadError('Este bloco já está sendo recebido.', 409);
      throw error;
    }

    const temporaryPath = path.join(this.sessionPath(id), `${index}.${crypto.randomUUID()}.tmp`);
    let fileHandle;
    try {
      ...
      for await (const chunk of request) {
        ...
        if (received > expectedSize) throw new UploadError('O bloco recebido é maior que o esperado.');
        hash.update(chunk);
      ...
      if (received !== expectedSize) throw new UploadError('O bloco chegou incompleto.', 422);
      if (!safeEqual(hash.digest('hex'), expectedHash)) {
        throw new UploadError('Falha de integridade no bloco. Tente enviá-lo novamente.', 422);
      }
  ...
  async finalize(id, registerFile) {
    ...
    try {
      ...
      if (error.code === 'EEXIST') throw new UploadError('O arquivo já está sendo finalizado.', 409);
      throw error;
    }
    ...
    try {
      ...
      if (uploadedChunks.length !== session.totalChunks) {
        ...
        throw new UploadError(`Ainda faltam ${missing} bloco${missing === 1 ? '' : 's'}.`, 409);
      }
      ...
      session = { ...session, status: 'completed', completedAt: new Date().toISOString(), updatedAt: new Date().toISOString(), file };
      await this.writeSession(session);
  ...
  async cleanupExpired() {
    const entries = await fsp.readdir(this.chunkRoot, { withFileTypes: true }).catch(() => []);
    const now = Date.now();
    for (const entry of entries) {
      ...
      try {
        ...
        const reference = Math.max(new Date(session.updatedAt || session.createdAt).getTime(), directoryStat.mtimeMs);
        if (now - reference > this.sessionTtlMs) await this.cancel(entry.name);
      } catch { /* Uma pasta inválida não deve interromper a limpeza das demais. */ }
    }
  }
  ...
  async findActiveSession(fingerprint) {
    ...
    for (const entry of entries) {
      ...
      try {
        const session = await this.readSession(entry.name);
        if (session.status === 'active' && session.fingerprint === fingerprint) return session;
      } catch { /* Ignora sessões danificadas. */ }
    }
    return null;
  }

  async listValidChunks(session) {
    const entries = await fsp.readdir(this.sessionPath(session.id), { withFileTypes: true });
    const valid = [];
    for (const entry of entries) {
      ...
      if (index >= 0 && index < session.totalChunks
        && await fileHasSize(path.join(this.sessionPath(session.id), entry.name), this.expectedChunkSize(session, index))) {
        valid.push(index);
      }
    }
  ...
  async assemble(session, target) {
    ...
    try {
      for (let index = 0; index < session.totalChunks; index += 1) {
        const partPath = path.join(this.sessionPath(session.id), `${index}.part`);
        const input = fs.createReadStream(partPath, { highWaterMark: 1024 * 1024 });
    ...
    if (total !== session.size) {
      ...
      throw new UploadError('O arquivo montado não corresponde ao tamanho original.', 422);
    }
  ...
  async removePartFiles(session) {
    ...
    await Promise.all(entries
      .filter((entry) => entry.isFile() && /\.(part|tmp|lock)$/.test(entry.name))
  ...
  sessionPath(id) {
    if (!UUID_PATTERN.test(String(id))) throw new UploadError('Sessão de upload inválida.');
    return path.join(this.chunkRoot, id);
  }

  async readSession(id) {
    try {
      return JSON.parse(await fsp.readFile(path.join(this.sessionPath(id), 'session.json'), 'utf8'));
    } catch (error) {
      if (error.code === 'ENOENT') throw new UploadError('Sessão de upload não encontrada.', 404);
      throw error;
    }
  }
...
async function fileHasSize(filename, size) {
  try { return (await fsp.stat(filename)).size === size; }
  catch (error) { if (error.code === 'ENOENT') return false; throw error; }
}

async function removeIfExists(filename) {
  try { await fsp.unlink(filename); }
...
async function writeAll(fileHandle, buffer) {
  ...
  while (offset < buffer.length) {
    ...
    if (bytesWritten <= 0) throw new UploadError('O disco não aceitou todos os bytes do arquivo.', 507);
    offset += bytesWritten;
  }
}
